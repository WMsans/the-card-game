...
class _TrapAnchorStub extends Control:
	var _player_trap: Control
	var _opp_trap: Control

func test_trap_set_resolves_to_trap_pile_anchor() -> void:
	var stub := _TrapAnchorStub.new()
	add_child(stub)
	auto_free(stub)
